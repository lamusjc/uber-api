...
module.exports = function (req, res) {
  ...
  if (req.roleName == "ADMIN") {
    query =
      ...
      "inner join products on bill_detail.products_id = products.products_id ORDER BY bill.bill_id DESC;";
  } else {
    query =
      "SELECT * FROM bill_detail " +
      "inner join bill on bill_detail.bill_id = bill.bill_id " +
      ...
      "where bill.users_id = ? OR bill.bill_users_received = ? ORDER BY bill.bill_id DESC;";
  }
  connection.query(query, [req.userId, req.userId], (error, result, fields) => {
    if (error) {
      response = {
